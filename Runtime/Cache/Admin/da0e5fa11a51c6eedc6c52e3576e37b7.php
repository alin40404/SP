<?php if (!defined('THINK_PATH')) exit();?><div id="goodsvarietySet1">
    <div>
        ...
        <select name="<?php echo ($pref); ?>_prvSelect" class="span2" rel="tooltip" data-placement="right" title="省/直辖市">
            ...
            <option>---暂无数据---</option>
			<?php }else{ if(!empty($aid)){ ?>
			 <option value="<?php echo ($aname); ?>_<?php echo ($aid); ?>"><?php echo ($aname); ?></option>
			<?php
 } foreach($assortment as $key=>$value){ if($aid!=$value['aid']){ ?>
            <option value="<?php echo $value['aname'],'_',$value['aid']; ?>"><?php echo $value['aname']; ?></option>
            <?php } } }?>
        </select>
    ...
    <div>
      ...
      <div class="alert-info" style=" margin: 5px 3px; padding: 2px; text-align: center;">
        ...
        <table class="table table-hover table-condensed" style="font-size: small;">
            ...
            <tbody>
                <?php  foreach($data as $id=>$array){ $id=$array['vid']; $name=$array['vname']; ?>
                <tr>
                    <td>
...
<script type="text/javascript">
    	...
    	pageClickByData( ".<?php echo ($pref); ?>_page",{aid:"<?php echo ($aid); ?>",aname:"<?php echo ($aname); ?>",key:"<?php echo ($search); ?>"}, "<?php echo ($replaceId); ?>", host + "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=showAll");
    	...
    	mutilDel("<?php echo ($pref); ?>_deleteAll", "<?php echo ($pref); ?>_checkbox", "<?php echo ($pref); ?>_alert_checkbox_choose","<?php echo ($pref); ?>_deleteAllModal", "<?php echo ($pref); ?>_to_deleteAll", host+ "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=deleteAll", {p :  "<?php echo ($pgInfo['nowPage']); ?>"}, "<?php echo ($replaceId); ?>");
    	 ...
    	 //添加商品名称
        singleBtn("<?php echo ($pref); ?>_btnAdd", "<?php echo ($pref); ?>_inputAdd", host + "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=addByName", {p :  "<?php echo ($pgInfo['nowPage']); ?>",aid:"<?php echo ($aid); ?>"}, "<?php echo ($replaceId); ?>");
    ...
    singleBtn("<?php echo ($pref); ?>_btnSearch", "<?php echo ($pref); ?>_inputSearch", host + "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=showAll", {
        ...
        aid:"<?php echo ($aid); ?>",aname:"<?php echo ($aname); ?>"
    },  "<?php echo ($replaceId); ?>");
...
$("select[name='<?php echo ($pref); ?>_prvSelect']").change(function(){
	...
	ajaxRequest("post", url, {aid:id,aname:name}, "",  "#<?php echo ($replaceId); ?>");
})
