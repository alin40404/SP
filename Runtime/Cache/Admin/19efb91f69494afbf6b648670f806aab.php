<?php if (!defined('THINK_PATH')) exit();?><div id="zoneSet1">
    <div>
        省/直辖市：
        <select name="<?php echo ($pref); ?>_prvSelect" class="span2" rel="tooltip" data-placement="right" title="省/直辖市">
		 ...
		 ,市/区：
            ...
            <option value="<?php echo $value['cname'],'_',$value['cid']; ?>"><?php echo $value['cname']; ?></option>
            <?php } } }?>
        </select>
    </div>
    <div>
    ...
    <div class="alert-info" style=" margin: 5px 3px; padding: 2px; text-align: center;">
        ...
        <table class="table table-hover table-condensed" style="font-size: small;">
            ...
            <tbody>
                <?php  foreach($data as $id=>$array){ $id=$array['zid']; $name=$array['zname']; ?>
                <tr>
                    <td>
...
<script type="text/javascript">
    	...
    	mutilDel("<?php echo ($pref); ?>_deleteAll", "<?php echo ($pref); ?>_checkbox", "<?php echo ($pref); ?>_alert_checkbox_choose","<?php echo ($pref); ?>_deleteAllModal", "<?php echo ($pref); ?>_to_deleteAll", host+ "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=deleteAll", {p :  "<?php echo ($pgInfo['nowPage']); ?>"}, "<?php echo ($replaceId); ?>");
    	  ...
    	  //添加商品名称
        singleBtn("<?php echo ($pref); ?>_btnAdd", "<?php echo ($pref); ?>_inputAdd", host + "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=addByName", {p :  "<?php echo ($pgInfo['nowPage']); ?>",id:"<?php echo ($cityid); ?>"}, "<?php echo ($replaceId); ?>");
 
$("select[name='<?php echo ($pref); ?>_prvSelect']").change(function(){
	var pStr=this.value;
	var arrP=pStr.split("_");
	var pname=arrP[0]
	var pid=arrP[1];
	
	var url= host + "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=showAll";
	ajaxRequest("post", url, {pid:pid,pname:pname}, "",  "#<?php echo ($replaceId); ?>");
})
...
$("select[name='<?php echo ($pref); ?>_citySelect']").change(function(){
	...
	var url= host + "g=<?php echo ($group); ?>&m=<?php echo ($module); ?>&a=showAll";
	ajaxRequest("post", url, {pid:"<?php echo ($prid); ?>",pname:"<?php echo ($pname); ?>",cid:id,cname:name}, "",  "#<?php echo ($replaceId); ?>");
})
