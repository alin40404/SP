<?php
class MarketProvinceModel extends Model {
	protected $table = "market_province";
	public function setPageList($p = 1, $pageNum = 5) {
		while ( true ) {
			$result = $this->order ( "pid" )->page ( "$p,$pageNum" )->select ();
			if ($result == null && $p > 1) {
				$p --;
			} else {
				break;
	...
	public function getAll(array $limit = array()) {
		if ($limit == null) {
			return $this->order ( "pid" )->select ();
		}
		return $this->order ( "pid" )->limit ( "$limit[begin],$limit[offset]" )->select ();
	}
	...
	public function selectPreciseByName($name) {
		$result = $this->where ( "pname ='$name'" )->select ();
		return $result;
	}
	/**
	 * 根据id编辑
	...
	public function editById($id, $name) {
		...
		if ($result) {
			...
			$data ['pname'] = $name;
			$result = $this->where ( "pid='$id'" )->save ( $data );
			if ($result == 1) {
				return true;
			} else {
				return false;
