...
SNS.provide(function() {
	var B = location.hostname.indexOf(".daily.") !== -1
	...
	var A = {
		...
		domain : {
			assets : B ? "assets.daily.taobao.net" : "a.tbcdn.cn",
			server : B ? "daily.taobao.net" : "taobao.com"
		},
		isTBDomain : function() {
		...
		normalize : function(C) {
			if (A.isDaily()) {
				C = C.replace("a.tbcdn.cn", "assets.daily.taobao.net").replace(
						"taobao.com", "daily.taobao.net")
			} else {
				C = C.replace("assets.daily.taobao.net", "a.tbcdn.cn").replace(
						"daily.taobao.net", "taobao.com")
			}
			return C
		}
	};
	...
	var A = {
		SM : function() {
			if (!window.UA_Opt || !window.ua) {
				...
				KISSY.getScript("http://acjs.aliyun.com/actionlog/js/ua.js",
						function() {
							try {
								UA_Opt.reload()
							} catch (B) {
...
SNS
		.provide(function() {
			var K = SNS;
			var E = location.hostname.indexOf("taobao.net") > -1 ? "http://assets.daily.taobao.net/p"
					: "http://a.tbcdn.cn/p";
			var L = {
				base : "",
				alias : {
				...
				map : [],
				load : [],
				timestamp : Math.floor(new Date().getTime() / 86400000 / 3),
				skin : true
			};
			var H = {};
			var I;
			...
			function P(R, T) {
				...
				R = R.replace(/([^:\/])\/+/g, "$1/");
				if (R.indexOf(".css") == -1 && R.indexOf("?") == -1
						&& R.indexOf("#") == -1) {
					R += ".js"
				}
				R += "?t=" + L.timestamp;
	...
	SNS
			.config({
				load : function(A, B) {
					...
					if (A == "kissy/overlay" && window.KISSY
							&& KISSY.version < "1.2.0") {
						return
					}
	...
	SNS.mix(B.prototype,
			{
				...
				_createWidget : function(C) {
					this._initConfigs(C);
					this._createElement(this.configs.element
							|| this.configs.ui.element);
					this._create();
				...
				destroy : function() {
					if (this.element) {
						...
						KISSY.DOM.removeData(this.element, "data-"
								+ this.baseClassName);
...
SNS
		.provide({
			checkLogin : function() {
				...
				var C = function(F) {
					var E, D;
					if ((D = String(document.cookie).match(
							new RegExp("(?:^| )" + F + "(?:(?:=([^;]*))|;|$)")))) {
				...
				var A = (new Date().getTime() - SNS.config("logintime"))
						/ (1000 * 60);
				if (A > 30) {
					B = false
				}
			...
			fetchToken : function(F, E) {
				...
				if (SNS.isTBDomain() || E) {
					SNS
							.ajax({
								_fetchToken : true,
								url : "http://comment.jianghu.taobao.com/json/token.htm",
								success : function(G) {
									G = G
											&& G.replace(
													/^[\s\xa0]+|[\s\xa0]+$/g,
			...
			login : function(F, E) {
				...
				SNS.ui("login", {
					logintime : SNS.config("logintime"),
					force : E || false,
					ui : {
						isIframe : B,
						type : "iframePopup",
...
SNS.provide(function() {
	...
	A.goldlog = function(B) {
		new Image().src = "http://log.mmstat.com/" + B + "?cache="
				+ new Date().getTime()
	};
	...
	SNS.each({
		...
		get : "sns/core/ajax",
		post : "sns/core/ajax",
		overlayAdapt : "sns/core/overlay",
		placeholder : "placeholder" in document.createElement("input") ? ""
				: "sns/util/placeholder",
		maxlength : "maxLength" in document.createElement("textarea") ? ""
				: "sns/util/maxlength"
	}, function(C, B) {
		A[B] = function() {
			if (C == "") {
