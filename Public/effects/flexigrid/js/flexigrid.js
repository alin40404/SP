...
(function ($) {
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				if($.isPlainObject(p.query)){
					...
					param = jQuery.param(param)+'&'+jQuery.param(p.query);
			...
			doSearchMy: function (pram) {
				query_arr = pram.split(',');
				var pram_obj = {};
				$.each(query_arr, function(i,n){
					pram_obj[n] = $('#'+n).val();	
				});
				p.query = pram_obj;
		...
		if (p.usepager) {
			...
			//add search button
			if (p.searchitems && $.isArray(p.searchitems)) {
  ...
  $.fn.flexSearch = function(p) { // function to search grid
	return this.each( function() {
		this.grid.doSearchMy(p.searchitems);
	});
  }; //end flexSearch
})(jQuery);
